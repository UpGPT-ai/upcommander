...
export interface DataConnector {
  /** Unique name identifying this connector (e.g. 'google-ads-api', 'stripe-webhook'). */
  name: string;
 ...
 */
export function registerConnector(name: string, connector: DataConnector): void {
 ...
 */
export function getConnector(name: string): DataConnector | null {
...
export abstract class BaseConnector implements DataConnector {
   ...
   */
  async *stream(params: Record<string, unknown>): AsyncGenerator<unknown> {
  ...
  protected assertConnected(): void {
    if (!this.isConnected()) {
      throw new Error(
        `Connector "${this.name}" is not connected. Call connect() before query() or stream().`
      );
    }
  }
}
