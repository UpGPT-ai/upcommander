...
function formatFact(fact: CoreFact): string {
...
export function assembleContext(
  ...
  if (recentSessions.length > 0) {
    ...
    for (const session of recentSessions) {
      level2Parts.push(
        `### ${session.created.slice(0, 10)} — ${session.worker} on "${session.project}"`
      );
...
export function recallMemory(
  ...
  const results: Array<{ type: string; title: string; excerpt: string }> = [];
  ...
  for (const learning of learnings) {
    ...
    if (results.length >= limit * 2) break; // Over-fetch then trim
  }
