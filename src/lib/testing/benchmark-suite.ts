 ...
 */
export function loadBenchmarkSuite(suitePath: string): BenchmarkSuite {
  if (!existsSync(suitePath)) {
    throw new Error(`Benchmark suite not found: ${suitePath}`);
  }

  const raw = readFileSync(suitePath, 'utf-8');
  const parsed = JSON.parse(raw) as Partial<BenchmarkSuite>;
 ...
 */
export function saveBenchmarkSuite(suite: BenchmarkSuite, outputPath: string): void {
 ...
 */
export function createBenchmarkFromFindings(
  findings: Finding[],
  domain: string,
  name: string,
  dataPath = '',
  ...
  const knownFindings: KnownFinding[] = findings.map((f) => ({
    type: f.type,
    title: f.title,
    severity: f.severity,
  ...
  const knownCrossRefs: KnownCrossRef[] = [];
 ...
 */
export function benchmarkNameFromPath(suitePath: string): string {
