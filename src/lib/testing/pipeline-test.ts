...
export interface PipelineConfig {
  ...
  workerCount: number;
  verificationLevel: number; // 0=none, 1=self-check, 2=peer, 3=synthesis, 4=completeness, 5=evidence-walk
  model?: string;            // MODEL_PRESETS key
}
...
export interface ConfigComparison {
  ...
  best_by_quality: string;     // config name with most unique findings
  best_by_cost: string;        // config name with lowest cost per finding
  best_by_speed: string;       // config name with lowest duration
  recommendation: string;      // config name balancing all factors
}
 ...
 */
function estimateCost(tokens: number, verificationLevel: number): number {
...
function computeBalancedScore(
  ...
  allResults: PipelineTestResult[],
): number {
  const maxQuality = Math.max(...allResults.map((r) => r.unique_findings));
  const minCostPerFinding = Math.min(
    ...allResults.map((r) =>
      r.unique_findings > 0 ? r.cost_usd / r.unique_findings : Infinity
    ),
  );
  ...
  const costPerFinding =
    result.unique_findings > 0 ? result.cost_usd / result.unique_findings : Infinity;
  const costScore =
    costPerFinding === Infinity || minCostPerFinding === 0
      ? 0
...
export async function runPipelineTest(
  ...
  for (const config of configs) {
    ...
    const tokensUsed = assignments.reduce(
      (sum, assignment) =>
        sum + assignment.chunks.reduce((s, c) => s + c.token_estimate, 0),
      0,
    );
    ...
    const stubFindings: Finding[][] = assignments.map((assignment) =>
      assignment.chunks.map((chunk) => ({
  ...
  // Determine winners
  const bestByQuality = results.reduce((best, r) =>
    r.unique_findings > best.unique_findings ? r : best
  );
  ...
  const bestBySpeed = results.reduce((best, r) =>
    r.duration_ms < best.duration_ms ? r : best
  );

  const bestBalanced = results.reduce((best, r) =>
    computeBalancedScore(r, results) > computeBalancedScore(best, results) ? r : best
  );
 ...
 */
export function saveTestResults(results: ConfigComparison, outputDir: string): void {
