...
export interface BudgetAlert {
  threshold: number;     // 0.75, 0.90, or 1.0
  triggered: boolean;
  triggered_at?: string;
}
...
export interface ProjectBudget {
  ...
  total_usd: number;
  spent_usd: number;
  allocated: Record<string, number>;  // worker role → budget allocation (USD)
  alerts: BudgetAlert[];
  created: string;
  updated: string;
}
...
export interface CostEstimate {
  ...
  breakdown: Record<string, number>;  // worker role → estimated cost
}
...
export interface BudgetCheckResult {
  remaining: number;
  percentage: number;      // 0-100, how much of the budget has been used
  exceeded: boolean;
  alerts: BudgetAlert[];
}
...
function budgetFilePath(projectPath: string): string {
  return join(BUDGETS_DIR, `${projectSlug(projectPath)}.json`);
}

function spendLogPath(projectPath: string): string {
...
const DEFAULT_THRESHOLDS: number[] = [0.75, 0.90, 1.0];

function defaultAlerts(): BudgetAlert[] {
...
function readBudgetFile(projectPath: string): ProjectBudget | null {
  const file = budgetFilePath(projectPath);
  if (!existsSync(file)) return null;
  try {
    return JSON.parse(readFileSync(file, 'utf8')) as ProjectBudget;
  } catch {
    return null;
  }
}

function writeBudgetFile(budget: ProjectBudget): void {
  ensureBudgetsDir();
 ...
 */
export function getBudget(projectPath: string): ProjectBudget | null {
 ...
 */
export function recordSpend(projectPath: string, record: SpendRecord): void {
  ensureBudgetsDir();
  ...
  if (!budget) return;  // No budget to update
  ...
  const previousAlerts = JSON.parse(JSON.stringify(budget.alerts)) as BudgetAlert[];
 ...
 */
export function checkBudget(projectPath: string): BudgetCheckResult {
  const budget = readBudgetFile(projectPath);
 ...
 */
const TEMPLATE_COST_PROFILES: Record<string, {
  costPerTaskUsd: number;
  workerRoles: string[];
  taskMultiplier: number;   // tasks generated per document page
}> = {
  dev: {
    costPerTaskUsd: 0.12,
 ...
 */
export function estimateCost(documentPages: number, template: string): CostEstimate {
  ...
  return {
    ...
    breakdown:            Object.fromEntries(
      Object.entries(breakdown).map(([k, v]) => [k, Math.round(v * 1000) / 1000])
    ),
  };
}
 ...
 * ```
 */
export function getBudgetSummary(projectPath: string): string {
  const budget = readBudgetFile(projectPath);
 ...
 */
export function readSpendLog(projectPath: string): SpendRecord[] {
  const logFile = spendLogPath(projectPath);
  ...
  try {
    ...
    for (const line of lines) {
      try {
        records.push(JSON.parse(line) as SpendRecord);
      } catch {
 ...
 */
export function spendByWorker(projectPath: string): Record<string, number> {
 ...
 */
function emitBudgetAlert(budget: ProjectBudget, alert: BudgetAlert): void {
  ...
  if (alert.threshold >= 1.0) {
    console.error(
      `[Budget] HARD STOP: Project "${proj}" has exceeded its budget. ` +
      `Spent ${spent} of ${total}. Further API executions are blocked.`
    );
  } else if (alert.threshold >= 0.90) {
    console.warn(
      `[Budget] WARNING: Project "${proj}" is at ${pct}% of budget. ` +
      `Spent ${spent} of ${total}. Approaching hard cap.`
    );
  } else {
    console.warn(
      `[Budget] NOTICE: Project "${proj}" has used ${pct}% of its budget. ` +
 ...
 */
export function assertBudgetAvailable(projectPath: string): void {
  const check = checkBudget(projectPath);
  if (check.exceeded) {
    throw new Error(
      `Budget exceeded for project "${basename(projectPath)}". ` +
