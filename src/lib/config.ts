...
export interface CommanderConfig {
  ...
  host: string;
  projectRoots: string[];
  sessions: Record<string, string>; // session name → project path
}
...
export function loadConfig(): CommanderConfig {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<CommanderConfig>;
    return {
      ...DEFAULT_CONFIG,
      ...parsed,
 ...
 */
export function saveConfig(config: Partial<CommanderConfig>): void {
  ensureConfigDir();
