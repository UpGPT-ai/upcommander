 ...
 */
function getWindowPath(sessionName: string, windowName: string): string {
  try {
    return execSync(
      `tmux display-message -t "${sessionName}:${windowName}" -p "#{pane_current_path}"`,
      { encoding: 'utf8', timeout: 3000 }
    ).trim();
  } catch {
 ...
 */
export function saveAllSessions(): SessionSnapshot {
  ensureSnapshotsDir();
 ...
 */
export function restoreAllSessions(snapshot?: SessionSnapshot): void {
  let snap = snapshot;
  ...
  if (!snap) {
    ...
    try {
      ...
      throw new Error(
        'Failed to parse snapshot: ' + (err instanceof Error ? err.message : String(err))
      );
    }
  }
  ...
  for (const sess of snap.sessions) {
    ...
    if (!sessionExists) {
      ...
      execSync(
        `tmux new-session -d -s "${sess.name}" -n "${firstWindowName}" -c "${workingDir}"`,
        { timeout: 5000 }
      );

      // Launch claude in the first window
      execSync(
        `tmux send-keys -t "${sess.name}:${firstWindowName}" "claude" Enter`,
        { timeout: 3000 }
      );
      ...
      for (let i = 1; i < sess.windows.length; i++) {
        ...
        execSync(
          `tmux new-window -t "${sess.name}" -n "${win.name}" -c "${workingDir}"`,
          { timeout: 3000 }
        );
        execSync(
          `tmux send-keys -t "${sess.name}:${win.name}" "${win.command || 'claude'}" Enter`,
          { timeout: 3000 }
        );
      }
    }
...
export function listSnapshots(): Array<{
  ...
  const results: Array<{ file: string; timestamp: string; sessions: number }> =
    [];
