...
function auditChainPath(projectPath: string): string {
  return join(coordDir(projectPath), 'audit-chain.jsonl');
}

function signaturesPath(projectPath: string): string {
  return join(coordDir(projectPath), 'signatures.json');
}

function retentionPath(projectPath: string): string {
...
function hashSignature(
  sig: Omit<ElectronicSignature, 'hash'>
): string {
  const canonical = JSON.stringify({
    signer: sig.signer,
    role: sig.role,
 ...
 */
export function createAuditChain(projectPath: string): void {
  ...
  const genesisEntry: Omit<AuditEntry, 'hash'> = {
    id: randomUUID(),
 ...
 */
function readAuditEntries(projectPath: string): AuditEntry[] {
...
export function appendAuditEntry(
  projectPath: string,
  entry: Omit<AuditEntry, 'id' | 'previous_hash' | 'hash'>
): AuditEntry {
  // Ensure chain exists
 ...
 */
export function verifyAuditChain(projectPath: string): ChainVerifyResult {
...
export function addSignature(
  projectPath: string,
  signature: Omit<ElectronicSignature, 'hash'>
): ElectronicSignature {
  const dir = coordDir(projectPath);
  ...
  const completed: ElectronicSignature = {
    ...signature,
 ...
 */
export function getSignatures(projectPath: string): ElectronicSignature[] {
  ...
  try {
    ...
    return JSON.parse(raw) as ElectronicSignature[];
  } catch {
    return [];
  }
}
 ...
 * @param projectPath Project root directory
 */
export function getRetentionPolicy(projectPath: string): RetentionPolicy | null {
  const path = retentionPath(projectPath);

  if (!existsSync(path)) return null;

  try {
    return JSON.parse(readFileSync(path, 'utf8')) as RetentionPolicy;
  } catch {
    return null;
  }
}
