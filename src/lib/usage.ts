...
export interface ProviderUsage {
  provider: string;            // 'anthropic' | 'openai' | 'google' | 'tmux'
  prompts: number;
  tokens_in: number;
  tokens_out: number;
  tokens_cached: number;
  cost_usd: number;
  models_used: Record<string, number>;  // model_id → prompt count
}
...
export interface SessionUsage {
  ...
  first_active: string;        // ISO timestamp
  last_active: string;         // ISO timestamp
  active_minutes: number;      // cumulative active time
...
export interface DailyUsage {
  ...
  tokens_cached: number;
  cost_usd: number;
  sessions_active: string[];   // which sessions were used
  by_provider: Record<string, ProviderUsage>;
}
...
export interface PromptEvent {
  ...
  provider?: string;           // 'anthropic' | 'openai' | 'google' | 'tmux'
  model?: string;              // e.g. 'claude-sonnet-4-6', 'gpt-4o', 'gemini-pro'
}
...
export function recordPrompt(event: PromptEvent): void {
  ensureUsageDir();
  const now = new Date().toISOString();
  const today = todayStr();

  // --- Update session usage ---
  const sessions = readJson<Record<string, SessionUsage>>(SESSIONS_FILE, {});

  const existing: SessionUsage = sessions[event.session] ?? {
  ...
  if (!existing.by_provider) existing.by_provider = {};
  ...
  // --- Update daily usage ---
  const daily = readJson<DailyUsage>(dailyFile(today), {
    date: today,
    prompts: 0,
    tokens_in: 0,
    tokens_out: 0,
  ...
  if (!daily.by_provider) daily.by_provider = {};
...
export function getUsageSummary(): UsageSummary {
  ...
  const emptyDaily = (date: string): DailyUsage => ({
    date,
    prompts: 0,
    tokens_in: 0,
    tokens_out: 0,
  ...
  // Today's usage
  const todayUsage = readJson<DailyUsage>(dailyFile(today), emptyDaily(today));
  ...
  let weekTokensOut = 0;
  let weekCost = 0;
  const weekByProvider: Record<string, ProviderUsage> = {};

  for (let i = 0; i < 7; i++) {
    const date = daysAgo(i);
    const daily = readJson<DailyUsage>(dailyFile(date), emptyDaily(date));
    weekDays.push(daily);
  ...
  let monthCost = 0;
  const monthByProvider: Record<string, ProviderUsage> = {};
  ...
  const sessions = readJson<Record<string, SessionUsage>>(SESSIONS_FILE, {});
  const bySession = Object.values(sessions).sort(
...
function _mergeProviders(
  target: Record<string, ProviderUsage>,
  source: Record<string, ProviderUsage>
): void {
...
export function getSessionUsage(sessionName: string): SessionUsage | null {
  ensureUsageDir();
  const sessions = readJson<Record<string, SessionUsage>>(SESSIONS_FILE, {});
