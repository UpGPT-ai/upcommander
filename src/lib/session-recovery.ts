...
export interface PaneState {
  ...
  window: string;
  lastOutput: string;
  lastOutputTime: number;      // epoch ms
  lastChangeTime: number;      // epoch ms when output last changed
...
export interface SwarmWorker {
  ...
  expectedOutputDir: string;   // where to check for completion
  expectedFiles: string[];     // filenames to look for
...
export interface SwarmState {
  ...
  startedAt: string;
  workers: SwarmWorker[];
  nextRefreshTime?: string;    // ISO timestamp of next rate limit refresh
}
...
let currentSwarmState: SwarmState | null = null;

/** Swarm state save interval handle */
let swarmSaveTimer: ReturnType<typeof setInterval> | null = null;

// ---------------------------------------------------------------------------
// Prompt cache
// ---------------------------------------------------------------------------

export function cachePrompt(session: string, window: string, prompt: string): void {
  promptCache.set(`${session}:${window}`, prompt);
}

export function getCachedPrompt(session: string, window: string): string | undefined {
 ...
 */
function capturePaneOutput(session: string, window: string): string | null {
  try {
    const raw = execSync(
 ...
 */
function isPanePidAlive(session: string, window: string): boolean {
  try {
 ...
 */
function parseRetryTime(output: string): string | null {
  for (const pattern of RETRY_TIME_PATTERNS) {
    ...
    else if (unit?.startsWith('hour') || unit?.startsWith('hr')) ms = num * 3_600_000;
    else ms = num * 60_000; // default to minutes
 ...
 */
function autoContinue(session: string, window: string): void {
  try {
 ...
 */
function resendCachedPrompt(session: string, window: string): boolean {
 ...
 */
function workerOutputExists(worker: SwarmWorker): boolean {
  if (!worker.expectedOutputDir || worker.expectedFiles.length === 0) {
    return false;
  }
  return worker.expectedFiles.every((f) =>
    existsSync(join(worker.expectedOutputDir, f))
  );
}
...
function scheduleRateLimitRecovery(
  ...
  window: string,
  refreshTime: string,
  callbacks: RecoveryMonitorCallbacks,
): void {
...
export function loadSwarmState(): SwarmState | null {
  try {
    if (!existsSync(SWARM_STATE_FILE)) return null;
 ...
 */
export function resumeSwarm(state: SwarmState): { resumed: string[]; completed: string[]; skipped: string[] } {
 ...
 */
export function runRecoveryCheck(callbacks: RecoveryMonitorCallbacks): PaneState[] {
  const tree = getSessionTree();
  const results: PaneState[] = [];
  ...
  for (const sess of tree.sessions) {
    for (const win of sess.windows) {
      ...
      if (paneState.state === 'completed') {
        ...
        if (currentSwarmState) {
          const swarmWorker = currentSwarmState.workers.find(
            (w) => w.name === win.name
          );
          if (swarmWorker) {
 ...
 */
export function getAllPaneStates(): PaneState[] {
 ...
 */
export function getCurrentSwarmState(): SwarmState | null {
 ...
 */
export function manualContinue(session: string, window: string): void {
  autoContinue(session, window);
}
 ...
 */
export function startRecoveryMonitor(
  callbacks: RecoveryMonitorCallbacks,
  intervalMs = 15_000,
): () => void {
