...
// ---------------------------------------------------------------------------
export const BUILT_IN_CHECKLISTS = {
    pharma: [
        'Module 2 summaries',
        'Module 3 CMC',
 ...
 */
export function selfCheck(finding) {
 ...
 */
export function peerReview(finding, allFindings) {
    const now = new Date().toISOString();
    const disagreements = [];
    const original = finding.confidence;
    let adjusted = original;
    ...
    const peers = allFindings.filter((f) => f.finding_id !== finding.finding_id &&
        f.worker !== finding.worker);
    for (const peer of peers) {
        // Check for cross-references pointing to the same finding_id (explicit link)
        const explicitLink = finding.cross_references.includes(peer.finding_id) ||
        ...
        const sevDiff = Math.abs(severityOrder[finding.severity] - severityOrder[peer.severity]);
        if (sevDiff >= 2) {
            passed = false;
 ...
 */
export function synthesisCheck(findings, crossRefGraph) {
    const now = new Date().toISOString();
    ...
    return findings.map((finding) => {
        ...
        const involvedConflicts = conflicts.filter((c) => finding.source_documents.some((doc) => c.reports.some((r) => r.source.document_id === doc.document_id)));
        for (const conflict of involvedConflicts) {
            const severityPenalty = conflict.severity === 'critical' ? 0.2
 ...
 * type or title.
 */
export function completenessAudit(findings, checklist) {
    const present_items = [];
    const missing_items = [];
    ...
    const coverage_pct = checklist.length === 0
        ? 100
 ...
 */
export function evidenceChainWalk(finding) {
    const broken = [];
 ...
 */
export function runVerification(findings, config) {
    const stages = [...config.stages].sort((a, b) => a - b);
    ...
    if (stages.includes(4)) {
        const checklist = config.requirementsChecklist ??
    ...
    if (config.escalateDisagreements) {
        for (const result of resultMap.values()) {
            const hasEscalation = result.disagreements.some((d) => d.suggested_action === 'escalate');
            if (hasEscalation) {
...
export function getVerificationSummary(results) {
    ...
    const completeness_pct = n > 0 ? Math.round(((n - (totalGaps > 0 ? 1 : 0)) / n) * 100) : 100;
    return {
        total_findings: n,
        verified: n,
        passed,
 ...
 */
export function saveVerificationResults(results, outputDir) {
...
function mergeStageResult(target, source, stage) {
