 ...
 */
function makeWorker(name, role, domain, tier = 2, model) {
...
export function getBuiltinTemplates() {
    return {
        ...
        dev: {
            ...
            workers: [
                makeWorker('orchestrator', 'Project Lead — plans and coordinates development', 'dev', 1, 'sonnet' // Default to Sonnet; use Opus only when explicitly requested
                ),
                makeWorker('backend', 'Backend engineer — APIs, database, business logic', 'dev', 2, 'sonnet'),
                makeWorker('frontend', 'Frontend engineer — UI components, pages, styling', 'dev', 2, 'sonnet'),
                makeWorker('tests', 'QA engineer — test suites, validation, coverage', 'dev', 2, 'sonnet'),
                makeWorker('deploy', 'DevOps — CI/CD, staging, production deployment', 'dev', 2, 'haiku' // deployment scripts are formulaic
                ),
            ],
            outputStructure: {
        ...
        research: {
            ...
            defaultModel: MODEL_PRESETS['sonnet'],
            workers: [
                makeWorker('orchestrator', 'Research Director — coordinates analysis campaign', 'research', 1, 'opus' // strategic synthesis needs deep reasoning
                ),
                makeWorker('competitors', 'Competitive Analyst — profiles and feature matrices', 'research', 2, 'sonnet'),
                makeWorker('complaints', 'Complaint Miner — Reddit, G2, Trustpilot, app stores', 'research', 2, 'haiku' // high-volume scanning, pattern matching
                ),
                makeWorker('pricing', 'Pricing Analyst — tier comparison, value mapping', 'research', 2, 'sonnet'),
                makeWorker('influencers', 'Influencer Scout — key voices, outreach targets', 'research', 2, 'haiku' // scanning and listing, not deep analysis
                ),
                makeWorker('synthesis', 'Research Synthesizer — final report compilation', 'research', 2, 'opus' // final synthesis requires highest quality
                ),
            ],
            outputStructure: {
        ...
        book: {
            ...
            defaultModel: MODEL_PRESETS['sonnet'],
            workers: [
                makeWorker('orchestrator', 'Editor-in-Chief — chapter planning and consistency', 'book', 1, 'opus' // narrative coherence needs deep reasoning
                ),
                makeWorker('outline', 'Outline Writer — structure, themes, chapter flow', 'book', 2, 'opus' // structural design is high-stakes
                ),
                makeWorker('writer-1', 'Chapter Writer — drafts assigned chapters', 'book', 2, 'sonnet'),
                makeWorker('writer-2', 'Chapter Writer — drafts assigned chapters', 'book', 2, 'sonnet'),
                makeWorker('writer-3', 'Chapter Writer — drafts assigned chapters', 'book', 2, 'sonnet'),
                makeWorker('editor', 'Copy Editor — consistency, tone, accuracy review', 'book', 2, 'opus' // editorial judgment needs highest quality
                ),
                makeWorker('citations', 'Fact Checker — claim verification, bibliography', 'book', 2, 'haiku' // lookup and verification tasks
                ),
                makeWorker('format', 'Formatter — final manuscript compilation', 'book', 2, 'haiku' // mechanical formatting
                ),
            ],
            outputStructure: {
        ...
        campaign: {
            ...
            defaultModel: MODEL_PRESETS['sonnet'],
            workers: [
                makeWorker('orchestrator', 'Campaign Director — channel coordination', 'campaign', 1, 'opus'),
                makeWorker('copy', 'Copywriter — landing pages, emails, ad variants', 'campaign', 2, 'sonnet' // creative writing needs sonnet-level
                ),
                makeWorker('social', 'Social Media Manager — platform-specific content', 'campaign', 2, 'haiku' // short-form content generation
                ),
                makeWorker('paid-search', 'PPC Specialist — keyword research, ad groups', 'campaign', 2, 'sonnet'),
                makeWorker('creative', 'Creative Director — visual asset briefs, image prompts', 'campaign', 2, 'sonnet'),
                makeWorker('analytics', 'Analytics Engineer — tracking, attribution, dashboards', 'campaign', 2, 'haiku' // structured config generation
                ),
            ],
            outputStructure: {
        ...
        video: {
            ...
            defaultModel: MODEL_PRESETS['sonnet'],
            workers: [
                makeWorker('orchestrator', 'Production Director — episode planning', 'video', 1, 'opus'),
                makeWorker('scripts', 'Scriptwriter — per-episode scripts', 'video', 2, 'sonnet' // creative writing
                ),
                makeWorker('storyboard', 'Storyboard Artist — shot lists, visual descriptions', 'video', 2, 'sonnet'),
                makeWorker('prompts', 'Prompt Engineer — generation prompts for Sora/Runway/Kling', 'video', 2, 'gemini-pro' // Google model for Veo/Imagen prompt optimization
                ),
                makeWorker('audio', 'Audio Director — voiceover scripts, music direction', 'video', 2, 'haiku' // structured script output
                ),
                makeWorker('assembly', 'Assembly Editor — asset manifests, edit instructions', 'video', 2, 'haiku' // mechanical assembly instructions
                ),
            ],
            outputStructure: {
        ...
        custom: {
            ...
            defaultModel: MODEL_PRESETS['sonnet'],
            workers: [
                makeWorker('orchestrator', 'Project Lead — coordinates all workers', 'custom', 1, 'opus'),
            ],
            outputStructure: {
 ...
 */
export function createCustomTemplate(name, description, workers) {
    const template = {
        name,
        description,
        workers,
 ...
 */
export function applyTemplate(template, projectPath, projectName) {
    ...
    for (const worker of template.workers) {
        ...
        writeFileSync(join(coordDir, 'MODEL.json'), JSON.stringify(modelConfig, null, 2), { mode: 0o600 });
    }
