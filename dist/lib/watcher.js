 ...
 */
export function startWatcher(projectPaths, callbacks) {
    if (projectPaths.length === 0) {
        // Nothing to watch — return a no-op stop function
        return () => { };
    }
    // Build glob patterns for each project path
    const patterns = projectPaths.map((p) => join(p, '.claude-coord', '**', 'STATUS.json'));
    ...
    function handleChange(filepath) {
        ...
        if (status.state === 'waiting_approval') {
            ...
            const { project, worker } = extractProjectWorker(filepath, projectPaths);
 ...
 */
function extractProjectWorker(filepath, projectPaths) {
    // Normalize separators
    const normalised = filepath.replace(/\\/g, '/');
    for (const projectPath of projectPaths) {
        const normalProject = projectPath.replace(/\\/g, '/');
        if (!normalised.startsWith(normalProject))
            continue;
