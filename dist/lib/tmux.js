...
export function getSessionTree() {
    try {
        ...
        const sessions = sessionNames.map((session) => {
            const windowsRaw = execSync(`tmux list-windows -t "${session}" -F "#{window_name}:#{pane_pid}:#{window_active}"`, { encoding: 'utf8', timeout: 5000 }).trim();
 ...
 */
export function sendKeys(session, window, prompt) {
    validateTarget(session, window);
    const sanitized = sanitizePrompt(prompt);
    execSync(`tmux send-keys -t "${session}:${window}" ${shellQuote(sanitized)} Enter`, { encoding: 'utf8', timeout: 10000 });
}
/**
 * Send a prompt to ALL windows in a session.
 */
export function broadcastToSession(session, prompt) {
 ...
 */
export function broadcastToAll(prompt) {
    const tree = getSessionTree();
    const sent = [];
 ...
 */
export function createSession(sessionName, projectPath, workers) {
    if (workers.length === 0) {
        throw new Error('At least one worker name is required');
    }
    // Create session with first worker as initial window
    execSync(`tmux new-session -d -s "${sessionName}" -n "${workers[0]}" -c "${projectPath}"`, { encoding: 'utf8', timeout: 10000 });
    // Launch claude in the first window
    execSync(`tmux send-keys -t "${sessionName}:${workers[0]}" "claude" Enter`, { encoding: 'utf8', timeout: 5000 });
    // Create remaining windows
    for (let i = 1; i < workers.length; i++) {
        execSync(`tmux new-window -t "${sessionName}" -n "${workers[i]}" -c "${projectPath}"`, { encoding: 'utf8', timeout: 5000 });
        execSync(`tmux send-keys -t "${sessionName}:${workers[i]}" "claude" Enter`, { encoding: 'utf8', timeout: 5000 });
    }
}
/**
 ...
 */
function validateTarget(session, window) {
