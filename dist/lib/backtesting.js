 ...
 */
export function calculateSharpe(returns, riskFreeRate = 0.05, periodsPerYear = 252) {
    if (returns.length < 2)
        return 0;
    ...
    const variance = excessReturns.reduce((sum, r) => sum + (r - mean) ** 2, 0) /
 ...
 */
export function calculateSortino(returns, riskFreeRate = 0.05, periodsPerYear = 252) {
    if (returns.length < 2)
        return 0;
    const periodicRiskFree = riskFreeRate / periodsPerYear;
    const excessReturns = returns.map((r) => r - periodicRiskFree);
    ...
    const downsideVariance = downsideReturns.reduce((sum, r) => sum + r ** 2, 0) / excessReturns.length;
 ...
 */
function checkTradeConstraints(config, tradeSize, currentEquity, currentDrawdown, dailyPnl) {
    let violations = 0;
    // Max position size
    if (currentEquity > 0) {
        const positionPct = Math.abs(tradeSize) / currentEquity;
        if (positionPct > config.constraints.max_position_pct)
            violations++;
    }
    // Max drawdown halt
    if (currentDrawdown > config.constraints.max_drawdown_pct)
        violations++;
    // Max daily loss
    if (dailyPnl < -config.constraints.max_daily_loss_usd)
        violations++;
    return violations;
}
// ---------------------------------------------------------------------------
 ...
 */
export function runBacktest(config, trades) {
    // Sort trades by date
    ...
    for (const trade of inRange) {
        ...
        const violations = checkTradeConstraints(config, tradeValueSize, equity, currentDrawdown, dailyPnl);
        if (violations > 0) {
    ...
    const avgTradePnl = appliedTrades.length > 0
...
export function compareStrategies(results) {
    ...
    const rows = results.map((r) => {
        const f = (n, decimals = 2) => n.toFixed(decimals);
 ...
 */
export function saveBacktestResult(result, outputDir) {
 ...
 */
export function loadBacktestResult(filePath) {
 ...
 */
export function equityCurveToReturns(curve) {
    const returns = [];
 ...
 */
export function calculateCalmar(result) {
