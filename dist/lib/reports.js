...
function pageEstimate(content) {
    return Math.max(1, Math.round(content.length / 3000));
}
function currentDate(override) {
...
function renderHtmlFinding(f, includeEvidence) {
    let html = `
    <tr>
...
function mdFinding(f, includeEvidence) {
    ...
    if (f.dollar_impact != null)
        md += `  |  **Impact:** ${formatDollar(f.dollar_impact)}`;
    md += `\n\n${f.description}\n`;
    if (includeEvidence && f.evidence_chain.length > 0) {
        md += `\n**Evidence:**\n\n`;
        for (const ev of f.evidence_chain) {
...
// ---------------------------------------------------------------------------
function domainSection(findings, domain, format) {
    switch (domain) {
        case 'sem':
...
function semSection(findings, format) {
    ...
    if (format === 'markdown') {
        ...
        md += `### Waste Report\n\n`;
        if (waste.length > 0) {
            md += mdTable(['Term / Campaign', 'Severity', 'Wasted Spend', 'Description'], waste.map((f) => [f.title, f.severity, formatDollar(f.dollar_impact), f.description.slice(0, 80)]));
        ...
        if (cannibalization.length > 0) {
            md += mdTable(['Campaign A', 'Severity', 'Impact', 'Description'], cannibalization.map((f) => [f.title, f.severity, formatDollar(f.dollar_impact), f.description.slice(0, 80)]));
        }
        else {
...
function legalSection(findings, format) {
    ...
    if (format === 'markdown') {
        ...
        if (privilege.length > 0) {
            md += mdTable(['Document', 'Privilege Type', 'Basis', 'Confidence'], privilege.map((f) => [
        ...
        hot.slice(0, 20).forEach((f, i) => {
            ...
            if (f.evidence_chain[0])
                md += `> *"${f.evidence_chain[0].excerpt}"*\n\n`;
        });
        md += `### Chronology\n\n`;
    ...
    hot.slice(0, 20).forEach((f, i) => {
        ...
        if (f.evidence_chain[0])
            html += `<div class="evidence-block"><em>&quot;${escapeHtml(f.evidence_chain[0].excerpt)}&quot;</em></div>`;
        html += `</p>`;
    });
...
function pharmaSection(findings, format) {
    ...
    if (format === 'markdown') {
        ...
        md += `### Gap Analysis\n\n`;
        if (gaps.length > 0) {
            md += mdTable(['Module / Section', 'Gap Type', 'Severity', 'Description'], gaps.map((f) => [f.title, f.type, f.severity, f.description.slice(0, 100)]));
        }
        else {
        ...
        if (crossRefBreaks.length > 0) {
            md += mdTable(['Reference', 'Severity', 'Description'], crossRefBreaks.map((f) => [f.title, f.severity, f.description.slice(0, 100)]));
        }
        else {
        ...
        if (jurisdictionConflicts.length > 0) {
            md += mdTable(['Conflict', 'Severity', 'Description'], jurisdictionConflicts.map((f) => [f.title, f.severity, f.description.slice(0, 100)]));
        }
        else {
...
function dueDiligenceSection(findings, format) {
    ...
    if (format === 'markdown') {
        ...
        md += `### Risk Matrix\n\n`;
        md += mdTable(['Risk', 'Type', 'Severity', 'Impact'], sorted.map((f) => [f.title, f.type, f.severity, formatDollar(f.dollar_impact)]));
        md += `\n### Deal-Breaker Report\n\n`;
        ...
        if (valuationRisks.length > 0) {
            md += mdTable(['Risk Factor', 'Estimated Impact', 'Confidence'], valuationRisks.map((f) => [f.title, formatDollar(f.dollar_impact), formatPct(f.confidence)]));
        }
        else {
...
function financialSection(findings, format) {
    const signals = findings.filter((f) => ['vol_mispricing', 'flow_anomaly', 'insider_signal', 'earnings_divergence'].includes(f.type));
    ...
    if (format === 'markdown') {
        ...
        if (signals.length > 0) {
            md += mdTable(['Signal', 'Type', 'Severity', 'Confidence', 'Impact'], signals.map((f) => [f.title, f.type, f.severity, formatPct(f.confidence), formatDollar(f.dollar_impact)]));
        }
        else {
        ...
        md += `\n### Risk Report\n\n`;
        if (regime.length > 0) {
            md += mdTable(['Regime Event', 'Severity', 'Impact'], regime.map((f) => [f.title, f.severity, formatDollar(f.dollar_impact)]));
        }
        else {
...
function predictionMarketSection(findings, format) {
    ...
    if (format === 'markdown') {
        ...
        if (estimates.length > 0) {
            ...
            md += mdTable(['Event', 'Probability', 'Confidence Interval', 'Confidence'], estimates.map((f) => [f.title, formatPct(f.confidence), '+/-10%', formatPct(f.confidence)]));
        }
        if (edges.length > 0) {
            md += `\n### Edge Calculations\n\n`;
            md += mdTable(['Edge', 'Expected Value', 'Confidence'], edges.map((f) => [f.title, formatDollar(f.dollar_impact), formatPct(f.confidence)]));
        }
        if (agreements.length > 0 || conflicts.length > 0) {
            md += `\n### Signal Agreement / Disagreement\n\n`;
            md += mdTable(['Signal', 'Type', 'Confidence'], [...agreements, ...conflicts].map((f) => [f.title, f.type, formatPct(f.confidence)]));
        }
        return md;
    }
 ...
 */
export function generateReport(findings, config, verification, crossRefSummary) {
    ...
    const narrative = `Analysis of ${docIds.size} document${docIds.size !== 1 ? 's' : ''} across the ` +
    ...
    if (config.format === 'markdown') {
        content = buildMarkdownReport(config, findings, top, narrative, sorted, verification, crossRefSummary, verSummary);
    }
    else {
        content = buildHtmlReport(config, findings, top, narrative, sorted, verification, crossRefSummary, verSummary);
    }
    return {
        title: config.title,
...
// HTML builder
// ---------------------------------------------------------------------------
function buildHtmlReport(config, findings, top, narrative, sorted, _verification, crossRefSummary, verSummary) {
...
// Markdown builder
// ---------------------------------------------------------------------------
function buildMarkdownReport(config, findings, top, narrative, sorted, _verification, crossRefSummary, verSummary) {
    const date = currentDate(config.date);
    const isExecutive = config.type === 'executive';
    const displayFindings = isExecutive ? top : sorted;
    let md = `# ${config.title}\n\n`;
    ...
    top.forEach((f, i) => {
        ...
        if (f.dollar_impact != null)
            md += ` — ${formatDollar(f.dollar_impact)}`;
        md += '\n';
    });
    md += '\n';
    // Domain-specific
    ...
    if (config.includeCrossRefs && crossRefSummary) {
        ...
        if (crossRefSummary.conflicts.length > 0) {
            md += `### Conflicts\n\n`;
            md += mdTable(['Entity', 'Type', 'Severity', 'Workers'], crossRefSummary.conflicts.map((c) => [
                c.entity_value,
                c.entity_type,
                c.severity,
 ...
 */
export function generateExecutiveSummary(findings, domain, title) {
 ...
 */
export function generateTechnicalReport(findings, domain, title) {
    return generateReport(findings, {
        format: 'html',
        type: 'technical',
        domain,
 ...
 */
export function generateDomainReport(findings, domain, title) {
    return generateReport(findings, {
        format: 'markdown',
