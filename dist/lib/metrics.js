 ...
 */
export function getProjectMetrics(projectPath, projectName) {
    ...
    if (orchestratorStatus)
        allStatuses.push(orchestratorStatus);
    let agentsRunning = 0;
    ...
    for (const s of allStatuses) {
        ...
        if (s.files_produced) {
            for (const f of s.files_produced) {
                if (!filesProduced.includes(f))
                    filesProduced.push(f);
            }
        }
    ...
    const elapsedMinutes = Math.floor((Date.now() - new Date(startedAt).getTime()) / 60_000);
    return {
        project: projectName,
        agentsRunning,
        agentsComplete,
 ...
 */
export function getSystemMetrics(projectPaths) {
    const tree = getSessionTree();
    const totalSessions = tree.sessions.length;
    const totalWindows = tree.sessions.reduce((sum, s) => sum + s.windows.length, 0);
    const entries = Array.isArray(projectPaths)
        ? projectPaths.map((p) => [basename(p), p])
        : Object.entries(projectPaths);
    const projects = entries.map(([name, path]) => getProjectMetrics(path, name));
