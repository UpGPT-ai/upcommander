...
async function extractXlsx(filePath) {
    try {
        ...
        for (const sheetName of workbook.SheetNames) {
            ...
            const csv = lib.utils.sheet_to_csv(sheet);
...
export function chunkDocument(doc, options = {}) {
    ...
    while (pos < text.length) {
        ...
        const headerPrefix = preserveHeaders && section ? `## ${section}\n\n` : '';
 ...
 */
export function distributeChunks(chunks, workerCount) {
    if (workerCount <= 0)
        throw new Error('workerCount must be a positive integer');
    if (chunks.length === 0)
        return Array.from({ length: workerCount }, (_, i) => ({ worker: i, chunks: [] }));
    ...
    for (let w = 0; w < workerCount; w++) {
        ...
        for (const chunk of primary) {
            ...
            // Add left neighbour
            if (idx > 0)
                extraIds.add(chunks[idx - 1].chunk_id);
            // Add right neighbour
        ...
        const combined = [...primary, ...overlap].sort((a, b) => chunkIndex.get(a.chunk_id) - chunkIndex.get(b.chunk_id));
...
export class DocumentRegistry {
     ...
     */
    assignChunk(documentId, chunkId, worker) {
     ...
     */
    markAnalyzed(documentId, chunkId) {
        const record = this.store.documents[documentId];
        if (!record)
            throw new Error(`Unknown document: ${documentId}`);
    ...
    getCoverage() {
        ...
        const analyzed = records.reduce((sum, r) => sum + r.chunks.filter((c) => c.analyzed).length, 0);
...
export async function ingestDirectory(dirPath) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            process.stderr.write(`[ingestion] Skipping ${fullPath}: ${err instanceof Error ? err.message : String(err)}\n`);
        }
    }
    return results;
}
