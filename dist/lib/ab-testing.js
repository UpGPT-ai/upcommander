...
function saveTest(test) {
    ensureDirs();
    writeFileSync(join(AB_TESTS_DIR, `${test.id}.json`), JSON.stringify(test, null, 2), { mode: 0o600 });
}
function readCurrentClaudeMd(worker) {
...
function makeVariant(worker, version, claudeMd) {
    return {
        id: randomUUID(),
        worker,
        version,
 ...
 */
export function createABTest(worker, challengerClaudeMd, minSample = 10) {
    ensureDirs();
    const existing = getActiveTest(worker);
    if (existing) {
        throw new Error(`Worker "${worker}" already has a running AB test: ${existing.id}`);
    }
 ...
 */
export function getActiveTest(worker) {
    if (!existsSync(AB_TESTS_DIR))
        return null;
    for (const file of readdirSync(AB_TESTS_DIR)) {
        if (!file.endsWith('.json'))
            continue;
        try {
            const test = JSON.parse(readFileSync(join(AB_TESTS_DIR, file), 'utf8'));
...
export function listABTests() {
    ...
    for (const file of readdirSync(AB_TESTS_DIR)) {
        if (!file.endsWith('.json'))
            continue;
        try {
            results.push(JSON.parse(readFileSync(join(AB_TESTS_DIR, file), 'utf8')));
        }
        catch {
            // Skip malformed files
        }
 ...
 */
export function recordABResult(testId, variant, quality, rejected) {
 ...
 */
export function evaluateTest(testId) {
    const test = readTest(testId);
 ...
 */
export function promoteWinner(testId) {
    ensureDirs();
    const test = readTest(testId);
    ...
    if (!evaluation.concluded || !evaluation.winner) {
        throw new Error(`AB test ${testId} cannot be concluded yet — not enough data (need ${test.minSample} tasks per variant)`);
    }
    const winner = evaluation.winner;
    const winnerVariant = winner === 'A' ? test.variantA : test.variantB;
    // Save winning CLAUDE.md
    writeFileSync(join(ROLES_DIR, `${test.worker}.md`), winnerVariant.claudeMd, { mode: 0o600 });
