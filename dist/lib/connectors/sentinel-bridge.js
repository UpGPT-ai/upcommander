 ...
 */
export async function parseSemCsv(filePath) {
    return new Promise((resolve, reject) => {
        ...
        rl.on('line', (rawLine) => {
            ...
            const row = {
                ...
                conversion_value: parseMetric(rowMap['conv. value'] ?? rowMap['conversion_value'] ?? rowMap['conv value'] ?? '0'),
            };
            rows.push(row);
        });
    ...
    for (const worker of workers) {
        ...
        switch (worker) {
            ...
            case 'opportunity-mapper':
                workerData = data.filter((r) => r.conversions > 0 || (r.impressions > 100 && r.conversions === 0));
                taskDescription =
 ...
 */
export function formatFindingsForSentinel(findings) {
    return findings.map((f) => {
        ...
        for (const link of f.evidence_chain) {
            ...
            if (termMatch?.[1])
                searchTerms.add(termMatch[1].trim());
            const campaignMatch = link.excerpt.match(/campaign[:\s]+"?([^",\n]+)"?/i);
            if (campaignMatch?.[1])
                campaigns.add(campaignMatch[1].trim());
        }
        ...
        for (const ref of f.cross_references) {
            if (ref.startsWith('kw:'))
                searchTerms.add(ref.slice(3));
            if (ref.startsWith('campaign:'))
                campaigns.add(ref.slice(9));
        }
        const sentinel = {
 ...
 */
export function writeFindingsReport(findings, outputPath) {
    // 1. Write raw findings JSON
    writeFileSync(join(outputPath, 'findings.json'), JSON.stringify(findings, null, 2), 'utf8');
    // 2. Write Sentinel-compatible findings JSON
    const sentinelFindings = formatFindingsForSentinel(findings);
    writeFileSync(join(outputPath, 'sentinel-findings.json'), JSON.stringify(sentinelFindings, null, 2), 'utf8');
...
class SentinelBridgeConnector extends BaseConnector {
    ...
    async connect(config) {
        ...
        if (!config['dataDir'] && !config['filePath']) {
            throw new Error('SentinelBridgeConnector requires "dataDir" or "filePath" in config.');
        }
        this.config = config;
        this.connected = true;
    }
    async query(params) {
        ...
        const filePath = params['filePath'] ??
            ...
            join(this.config['dataDir'], 'search-terms.csv');
 ...
 */
function buildMarkdownReport(findings, sentinelFindings) {
    ...
    for (const f of findings) {
        const impact = f.dollar_impact
            ...
            : '—';
        lines.push(`| ${f.finding_id} | ${f.type} | ${f.severity} | ${f.title} | ${impact} | ${f.worker} |`);
    }
...
//# sourceMappingURL=sentinel-bridge.js.map