...
import { parseSemCsv, distributeDataToWorkers, writeFindingsReport, } from '../lib/connectors/sentinel-bridge.js';
import { createFinding, mergeFindings, } from '../lib/output-schemas.js';
...
// ---------------------------------------------------------------------------
function extractFindingsFromResponse(responseText, worker) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function buildEvidenceChain(evidence) {
...
// ---------------------------------------------------------------------------
async function runWithApi(args, apiKey) {
    ...
    const workerTasks = packages.map(async (pkg) => {
        const worker = pkg.worker;
        ...
        const csvRows = [
            ...
            ...dataPreview.map((r) => `"${r.search_term}","${r.campaign}","${r.ad_group}",${r.impressions},${r.clicks},${r.cost},${r.conversions},${r.conversion_value}`),
        ].join('\n');
...
// ---------------------------------------------------------------------------
async function runTmuxMode(args) {
...
// ---------------------------------------------------------------------------
async function postProcess(args, allFindings) {
    ...
    for (const finding of merged) {
        ...
        const entities = extractEntities(`${finding.title} ${finding.description}`, 'sem', docRef, finding.worker);
...
// Output writing
// ---------------------------------------------------------------------------
function writeOutputs(args, findings) {
    ...
    writeFileSync(resolve(args.outputDir, 'executive-report.md'), report.content, 'utf8');
    log(`Written: executive-report.md`);
}
// ---------------------------------------------------------------------------
// Learning extraction
// ---------------------------------------------------------------------------
function saveLearnings(args, findings, totalCostUsd) {
...
// ---------------------------------------------------------------------------
function printSummary(args, findings) {
    const totalImpact = findings.reduce((s, f) => s + (f.dollar_impact ?? 0), 0);
    const bySeverity = {
...
//# sourceMappingURL=audit-csv.js.map