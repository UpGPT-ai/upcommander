...
import type { BridgeClient, TmuxWindow } from './bridge-client';
...
export class SessionDetailPanel {
  ...
  private currentWindow: TmuxWindow | undefined;
  ...
  static open(
    ...
    const panel = new SessionDetailPanel(
      context,
      client,
      sessionName,
      windowName,
  ...
  updatePaneOutput(output: string): void {
    if (!this.disposed) {
      void this.panel.webview.postMessage({
        type: 'paneOutput',
        output,
  ...
  sendFullLog(output: string): void {
    if (!this.disposed) {
      void this.panel.webview.postMessage({
        type: 'fullLog',
        output,
  ...
  static forwardPaneOutput(sessionName: string, windowName: string, output: string): void {
    const key = `${sessionName}:${windowName}`;
  ...
  static sendFullLogToPanel(sessionName: string, windowName: string, output: string): void {
    const key = `${sessionName}:${windowName}`;
    const panel = panels.get(key);
    if (panel && !panel.disposed) {
  ...
  static resubscribeAll(client: BridgeClient): void {
  ...
  private async _handleMessage(message: WebviewMessage): Promise<void> {
    switch (message.type) {
      case 'sendPrompt': {
        const prompt = (message.payload as { prompt: string }).prompt?.trim();
        if (!prompt) return;
        try {
      ...
      case 'approve': {
        try {
          ...
          void vscode.window.showInformationMessage(
            `Approved: ${this.sessionName}:${this.windowName}`
          );
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
      ...
      case 'deny': {
        try {
          ...
          void vscode.window.showInformationMessage(
            `Denied: ${this.sessionName}:${this.windowName}`
          );
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
...
interface WebviewMessage {
  type: 'sendPrompt' | 'approve' | 'deny' | 'subscribePanes';
  payload?: unknown;
}
